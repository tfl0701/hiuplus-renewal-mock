...
(function () {
  "use strict";
  var H = window.H;
  /* 진행 단계 — 2026-09 대표 지시로 다섯 단계가 됐다(마이페이지 진행 막대, ST-20260917-08).
   * 목업이 넷으로 남아 있어 한 화면은 넷·다른 화면은 다섯이 되던 것을 2026-09-18 에 맞췄다.
   * ★목업보다 «나중에 대표가 정한 것»이 위다. */
  var STEPS = ["신청", "접수확인", "준비중", "전달 예정", "개통완료"];
  /* 약관 — 우리는 LG U+ 대리점이고 직접 개통한다. 통신사에 손님 정보를 «넘기는» 것이
   * 아니므로 제3자 제공 동의는 받지 않는다(대표 2026-09-18 «우리는 대리점이야 직접개통이야
   * 제3자동의 안받아»). 개인정보 수집·이용 동의는 회원가입에서 이미 받는다.
   * 그래서 주문서 약관은 운영 화면과 같은 둘로 맞춘다. */
  ...
  H.views.order = function () {
    ...
    var body = `<div class="order-form">
      ...
      <section class="form-sec"><h2>인터넷도 같이</h2>
        ${H.netAskCard("", "인터넷도 같이 상담받을게요", "담당자가 결합 할인과 지원금을 따로 알려드려요.")}
      </section>
